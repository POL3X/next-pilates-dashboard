...
export async  function duplicateGroupAction(group: Group,taskColumn: Task[],compnayUuid: string){
    ...
    try{
        ...
        const taskColumns = taskColumn.length > 0 ? taskColumn.map(task => ({ uuid: task.uuid, waitList: task.waitList })) : [];
